...
export class BillingComponent implements OnInit{
      ...
      @Input() clearBillingEmails: any = '';
      @Input() GetBillingEmails: any[] = [];
      ...
      ngOnInit(): void {
        ...
        // Log the data you are about to use (for debugging)
              this.parentFormGroup.setControl(
                  this.controlKey ? this.controlKey : 'BillingDetails',
                  this.BillingForm
              );
          ...
          console.log('userDetailssss1',this.userDetails);
      }


      ngOnChanges(changes: SimpleChanges): void {
       if (changes['clearBillingEmails'] && changes['clearBillingEmails'].currentValue) {
           this.emails = [];
    this.BillingForm.get('BillingEmail')?.reset();
       }
        if(changes['GetBillingEmails'] && changes['GetBillingEmails'].currentValue){
           this.emails = [...this.GetBillingEmails]; 
             this.BillingForm.patchValue({
           BillingEmail: this.emails
          });
          // console.log('checkvalue',this.BillingForm.value)
          // console.log('GetBillingEmails',this.GetBillingEmails)
        }
      }
           ...
           return;
              ...
              if (value && emailControl?.valid) {
                this.emails.push(value);
                 this.BillingForm.patchValue({ BillingEmail: this.emails });
