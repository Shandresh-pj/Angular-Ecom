...
@Component({
    ...
    imports: [CommonModule,ReactiveFormsModule,MatTableModule,MatButtonModule,MatInputModule,
        MatSelectModule,MatIconModule,MatCardModule,MatCheckboxModule,
    ],
})
export class InvoicePdfComponent {
    ...
    async generatePDF(element?: any) {
        ...
        let invoiceData: any[] = [];
        try {
            const res: any = await firstValueFrom(this.commonService.getApi(`${this.apiRoute}/SeprateInvoicePdf/${element.Id}`,{}));
            this.invoiceList = res.data;
            for (const invoice of this.invoiceList) {
                const invoiceBills = invoice.InvoiceBills || [];
                const bookings = invoice.Booking || [];
                const customerTariffs = invoice.CustomerBillTariff || [];
                ...
                const customerTariff =customerTariffs.find((t: any) => t.BillId === bill.Id) || {};
                  return {
                    ...
                    TotalKms:parseFloat(bill.CloseKMS || '0') - parseFloat(bill.OpenKMS || '0') || 0,},
                ...
                for (const bill of enrichedBills) {
                    const invoiceDetail = {
                        ...
                        trip_type: bill.Booking?.RentalType?.Code || 'Garage',
                        ...
                        c_total_km: bill.Booking.TotalKms,
                        c_total_hrs: bill.Booking.TotalHrs,
                        ...
                        parking: Number(bill.CustomerBillTariff?.Parking) || 0,
                        permit: Number(bill.CustomerBillTariff?.Permit) || 0,
                        others: Number(bill.CustomerBillTariff?.Others) || 0,
                        cardamount:Number(bill.CustomerBillTariff?.CardAmt) || 0,
                    ...
                    const customerDetail = {
                        customer_name:bill.Booking?.CustomerCompany?.Name || '',
                        vendor_name: bill.Booking?.VendorCompany?.Name || '',
                        branch_address: `${bill.Booking?.VendorCompany?.CompanyAddress?.AddressLine_2 || ''}, ${bill.Booking?.VendorCompany?.CompanyAddress?.City ||''}, ${
                        ...
                        customer_address: `${bill?.Booking?.CustomerCompanyParent?.CustomerAddress?.AddressLine_2 || ''},${bill?.Booking?.CustomerCompanyParent?.CustomerAddress?.City || ''},${bill?.Booking?.CustomerCompanyParent?.CustomerAddress?.State || ''}`,
                        gst_no: bill.Booking?.CustomerCompany?.GstNo || '',
                        pan_no: bill.Booking?.CustomerCompany?.PanNo || '',
                        hsn_code: bill.Booking?.CustomerCompany?.HsnCode || '',
                        hsn_desc:bill.Booking?.CustomerCompany?.HsnDescription || '',
                        project_id: bill.Booking?.ProjectId || '',
                        request_id: bill.Booking?.RequestId || '',
                        employee_id: '',
                    };

                    const total =invoiceDetail.c_slab_rate + invoiceDetail.c_ex_km_amt + invoiceDetail.c_ex_hr_amt;
                    ...
                    invoiceData.push({invoiceDetail: [invoiceDetail],customerDetail,total,ceAllowance,
                        invoiceNumber: invoice.InvoiceNumber,
                        invoiceDate: invoice.InvoiceDate,
                    });
                }
            ...
            for (const data of invoiceData) {
                ...
                const htmlContent = this.generateInvoiceHTML(
                    ...
                    data.total,
                    data.ceAllowance,
                    data.invoiceDetail.reduce((sum: number, d: any) => sum + d.netAmt,0),
                    data.invoiceNumber,
                    data.invoiceDate
                );
            ...
            console.error('Error generating joined PDF:', error);
        } finally {
    ...
    generateInvoiceHTML(
        ...
        const permit = invoiceDetails.reduce(
            (sum, detail) =>
                sum +
                (Number(detail.permit) || 0) +
                (Number(detail.others) || 0),
            0
        );
    ...
    table tbody tr:nth-child(even) {
      ...
      <header style="padding: 5px 0; margin-bottom: 10px; border-bottom: 1px solid #AAAAAA; display: flex; justify-content: space-between; align-items: center; color:'#000">
        <div style="display: flex; align-items: center;">
          <img style="height: 30px; width: 150px;" src="Logo/TRYPDEKLOGO2.png" class="transition position-relative" alt="logo-icon" />
        </div>
        <div style="text-align: right;color: #000;">
          <h2 style="margin: 0; font-size: 15px;">${
              customerDetail.vendor_name
          }</h2>
          <div style="font-size: 13px; line-height: 14px;">${
              customerDetail.branch_address
          }</div>
          <div style="font-size: 13px; line-height: 14px;">${
              customerDetail.company_phone
          }</div>
          <div style="font-size: 13px; line-height: 14px;">${
              customerDetail.company_email
          }</div>
        </div>
      </header>
      <main style="font-family: Arial, sans-serif; font-size: 12px; color: #000;">
        ...
        <div id="details" style="margin-bottom: 15px;">
          <div style="padding-left: 6px; border-left: 6px solid #0087C3; font-size: 13px; width: 50%;">
            ...
            <h2 style="margin: 3px 0; font-size: 14px;">${customerDetail.customer_name }</h2>
        ...
        <div style="margin-bottom: 10px; font-size: 12px; display: flex; justify-content: space-between;">
          ...
          <div style="width: 25%; text-align: right;">
            Booked By: ${invoiceDetails[0]?.BookerBy}
          </div>
        </div>
        <table style="width: 100%;color: #000; border-collapse:  collapse; font-size: 11px; margin-top: 15px; border: 1px solid #ddd;">
          <thead>
            <tr style="background-color: #FFFFFF;">
              ...
              <th style="border: 1px solid #ddd; padding: 5px; text-align: left;">Vehicle # & Type</th>
              ...
              <th style="border: 1px solid #ddd; padding: 5px; text-align: left;">Extra Hours <br> Hours | Rate | Amt</th>
          ...
          <tbody>
            ${invoiceDetails
              ...
              <tr>
                ...
                <td style="border: 1px solid #ddd; padding: 5px;">${
                    detail.vehicle_no
                } ${detail.vehicle_model || ''}</td>
                <td style="border: 1px solid #ddd; padding: 5px;">${
                    detail.c_total_km || 0
                } | ${detail.c_total_hrs || 0} | ${Math.floor(
                        (detail.c_total_hrs || 0) / 24
                    )}</td>
                <td style="border: 1px solid #ddd; padding: 5px;">${
                    detail.c_slab_rate?.toFixed(2) || '0.00'
                }</td>
                <td style="border: 1px solid #ddd; padding: 5px;">${
                    detail.c_ex_hr || 0
                } | ${detail.c_ex_hour_rate?.toFixed(2) || '0.00'} | ${
                        detail.c_ex_hr_amt?.toFixed(2) || '0.00'
                    }</td>
                <td style="border: 1px solid #ddd; padding: 5px;">${
                    detail.c_ex_km || 0
                ...
                <td style="border: 1px solid #ddd; padding: 5px;">${(
                    Number(detail.c_slab_rate || 0) +
                    Number(detail.c_ex_km_amt || 0) +
                    Number(detail.c_ex_hr_amt || 0)
                ).toFixed(2)}</td>
              </tr>
            `
                )
        ...
        <div style="display: flex; justify-content: space-between;color: #000; margin-top: 15px; font-size: 12px;">
          <div style="width: 70%;">
            <div style="padding-left: 6px; border-left: 6px solid #0087C3;">
              <div><b>Open date:</b> ${new Date(
                  invoiceDetails[0]?.open_date
              ).toLocaleDateString('en-IN')}  <b>Close date:</b> ${new Date(
            invoiceDetails[invoiceDetails.length - 1]?.close_date
        ).toLocaleDateString('en-IN')}</div>
              <div><b>Open time:</b> ${
                  invoiceDetails[0]?.open_time
              }  <b>Close time:</b> ${
            invoiceDetails[invoiceDetails.length - 1]?.close_time
        }</div>
          ...
          <div style="width: 25%;">
            <table style="width: 100%; color: #000; border-collapse: collapse; border: 1px solid #AAAAAA; font-size: 10px;">
              <tr style="background-color: #f2f2f2;">
                ...
                <td style="padding: 4px; text-align: right;">${total.toFixed(
                    2
                )}</td>
              </tr>
              <tr>
                ...
                <td style="padding: 4px; text-align: right;">${
                    invoiceDetails[0]?.c_discount_amt?.toFixed(2) || '0.00'
                }</td>
              </tr>
              <tr style="background-color: #f2f2f2;">
                <td style="padding: 4px; text-align: left;">Parking</td>
                <td style="padding: 4px; text-align: right;">${TotalParking.toFixed(
                    2
                )}</td>
              </tr>
              <tr>
                <td style="padding: 4px; text-align: left;">Permit / Others</td>
                <td style="padding: 4px; text-align: right;">${permit.toFixed(
                    2
                )}</td>
              </tr>
              <tr style="background-color: #f2f2f2;">
                <td style="padding: 4px; text-align: left;">CE Allowance</td>
                <td style="padding: 4px; text-align: right;">${ceAllowance.toFixed(
                    2
                )}</td>
              </tr>
              <tr>
                <td style="padding: 4px; text-align: left;">CGST @ ${
                    invoiceDetails[0]?.cgst || '0.00'
                }%</td>
                <td style="padding: 4px; text-align: right;">${
                    TotalCgstAmt?.toFixed(2) || '0.00'
                }</td>
              </tr>
              <tr style="background-color: #f2f2f2;">
                <td style="padding: 4px; text-align: left;">SGST @ ${
                    invoiceDetails[0]?.sgst || '0.00'
                }%</td>
                <td style="padding: 4px; text-align: right;">${
                    TotalSgstAmt?.toFixed(2) || '0.00'
                }</td>
              </tr>
              <tr>
                <td style="padding: 4px; text-align: left;">IGST @  ${
                    invoiceDetails[0]?.igst || '0.00'
                }%</td>
                <td style="padding: 4px; text-align: right;">${
                    TotalIgstAmt?.toFixed(2) || '0.00'
                }</td>
              </tr>
              <tr style="background-color: #f2f2f2;">
                <td style="padding: 4px; text-align: left;">Invoice Amount</td>
                <td style="padding: 4px; text-align: right;">${
                    TotalInvoiceAmt?.toFixed(2) || '0.00'
                }</td>
              </tr>
              <tr>
                <td style="padding: 4px; text-align: left;">CARD Charges</td>
                <td style="padding: 4px; text-align: right;">${
                    Totalcardamt?.toFixed(2) || '0.00'
                }</td>
              </tr>
              <tr style="background-color: #f2f2f2;">
                <td style="padding: 4px; text-align: left;">Net Amount</td>
                <td style="padding: 4px; text-align: right;">${TotalNetAmt.toFixed(
                    2
                )}</td>
              </tr>
            </table>
        ...
        ${getIndianCurrency(TotalNetAmt || 0)}
          <div style="text-align: right; margin-top: 4px; font-weight: normal;">
            Invoice Amt: Rs. ${TotalInvoiceAmt?.toFixed(2) || '0.00'}<br>
            Card: Rs. ${invoiceDetails[0]?.c_card_amt?.toFixed(2) || '0.00'}<br>
            Net: Rs. ${(TotalNetAmt || 0).toFixed(2)}
          </div>
        </div>
        <div style="margin-top: 10px; font-size: 11px; color: #000; display: flex; justify-content: space-between;">
          <div>
            GSTIN/UIN #: ${customerDetail.gst_no}   Pan No: ${
            customerDetail.pan_no
        }<br>
            HSN/SAC Code: ${customerDetail.hsn_code} (Service Category: ${
            customerDetail.hsn_desc
        })<br>
            Bank Name: ICICI BANK / Account Number: 189905002884 / IFSC Code: ICIC0001899 <br>
            naration:${invoiceDetails[0]?.Naration}
          </div>
          <div style="text-align: right;">For Forza Enterprises Pvt. Ltd</div>
        </div>
      </main>
      <footer style="margin-top: 15px; font-size: 10px; text-align: center;color: #000; border-top: 1px solid #AAAAAA; padding-top: 8px;">
        ...
        <img style="max-width: 250px;" src="images/IC.jpeg" class="transition position-relative" alt="logo-icon" />
      </footer>
    `;
    }
    // async generatePDF(element?: any) {
    ...
    //                 vendor_name: bill.Booking?.VendorCompany?.Name || '',
    //                 branch_address: `${bill?.Booking?.VendorCompany?.CompanyAddress?.AddressLine_1 || ''}, ${bill?.Booking?.VendorCompany?.CompanyAddress?.AddressLine_2 || ''},${bill?.Booking?.VendorCompany?.CompanyAddress?.City || ''},${bill?.Booking?.VendorCompany?.CompanyAddress?.State ||''}`,
    //                 company_phone:bill.Booking?.VendorCompany?.PhoneNumber || '',
    //                 company_email: bill.Booking?.VendorCompany?.Email || '',
    //                 customer_address: `${bill?.Booking?.CustomerCompanyParent?.CustomerAddress?.AddressLine_1 || ''}, ${bill?.Booking?.CustomerCompanyParent?.CustomerAddress?.AddressLine_2 || ''},${bill?.Booking?.CustomerCompanyParent?.CustomerAddress?.City || ''},${bill?.Booking?.CustomerCompanyParent?.CustomerAddress?.State || ''}`,
    ...
    //         const margin = 10;
    //         for (const data of invoiceData) {
    //             const htmlContent = this.generateInvoiceHTML(data?.invoiceDetail,data?.customerDetail,data?.total,data?.ceAllowance,data?.invoiceDetail?.netAmt);
    //             const tempDiv = document.createElement('div');
    //             tempDiv.style.position = 'absolute';
    //             tempDiv.style.left = '-9999px';
    ...
    //             document.body.appendChild(tempDiv);
    //             const scale = 2;
    //             const canvas = await html2canvas(tempDiv, {scale: scale,useCORS: true,logging: false,backgroundColor: '#fff',});
    //             const imgData = canvas.toDataURL('image/png');
    //             const imgProps = pdf.getImageProperties(imgData);
    //             const pdfWidth = pageWidth - 2 * margin;
    //             const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width;
